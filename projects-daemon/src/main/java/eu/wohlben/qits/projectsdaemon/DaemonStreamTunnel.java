 ...
 * that WebSocket to a fresh TCP connection to {@link ProjectsApi} on loopback.
 *
 * <p><b>What this replaces.</b> {@code ProjectsApi} used to bind {@code 0.0.0.0} and be reachable
 * by DNS name from every other agent container on the shared docker network — every one of which
 * runs a coding agent over someone else's untrusted checkout, with an environment it can read the
 * shared API token straight out of. This daemon binds {@code 127.0.0.1} from its first version and
 * qits reaches it by asking for a stream over the control socket it already holds open. The port is
 * not a boundary that got stronger; it never existed.
 *
...
final class DaemonStreamTunnel {
  ...
  void open(String nonce, String path) {
    ...
    try {
      ...
      // The path arrives from the control socket, which is unauthenticated (anything on the
      // network can claim to be any project's daemon), so it is untrusted input that becomes an
      // address.
