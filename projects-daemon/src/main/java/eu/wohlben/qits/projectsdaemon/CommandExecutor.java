 ...
 *
 * <p>The one behaviour it must guarantee is that a failed spawn (a missing binary, say) still
 * produces a terminal {@link CommandExit} rather than throwing, so a caller's correlation future
 * always completes.
 */
public final class CommandExecutor {
  ...
  public static void run(RunCommand command, Consumer<DaemonMessage> emit) {
    ...
    if (command.env() != null) {
      ...
      // throws NPE on a null value (the wire codec can decode one), which would escape the catch
    ...
    try {
      ...
      // Spawn failed (bad argv, missing binary): report as stderr plus a conventional 127, so the
    ...
    Thread stderrPump =
        new Thread(
            ...
            "projects-daemon-cmd-stderr-" + correlationId);
  ...
  private static void pump(
    ...
    try (stream) {
      ...
      // Stream closed under us (the process died) — nothing more to read; the exit code carries the
      // outcome.
    }
  }
}
