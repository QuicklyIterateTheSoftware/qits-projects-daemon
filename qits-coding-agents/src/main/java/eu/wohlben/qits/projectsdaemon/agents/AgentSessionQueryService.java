...
/**
 * Assembles this container's agent sessions into the tree the session-history UI renders: one node
 ...
 * subagent rows.
 *
 * <p>It takes no owner argument, because there is none to take — every command in the store
 * belongs to this project by
...
public final class AgentSessionQueryService {
  ...
  /** The container's session tree — roots newest-first, fork children chronological. */
  public List<AgentSessionNodeDto> sessionTree() {
    ...
    // Fork edges nest children under their origin; an edge to a session this container never
