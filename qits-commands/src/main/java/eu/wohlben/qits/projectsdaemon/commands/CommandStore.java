 ...
 * starts and is gone when it stops. A recreated container has no command history, no logs and no
 ...
 * the equivalent lookups narrowed by an owner relation. Inside the container every command is this
 * project's by
...
public final class CommandStore {
   ...
   *
   * <p>Scoped structurally rather than asserted: the store only ever contains this container's
   * commands, so there is no owner to compare. It also means the check answers "no" for a session
   * this project drove in an earlier container — a resume of such a
